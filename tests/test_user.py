...
def get_sample_new_user_data(**overrides):
    user_data = {
        "username": USERNAME,
        "password": PASSWORD,
...
def get_sample_existing_user_data(**overrides):
    user_data = {
        "username": USERNAME,
        "display_name": DISPLAY_NAME,
        "email": EMAIL,
...
def get_user(client, user_id):
    return client.get("%s/%s" % (API_URL, user_id), headers=HEADERS)


class TestUserBase(BaseTestCase):
    def post_user(self, data):
        return post_user(self.test_client, data)

    def get_user(self, user_id):
        return get_user(self.test_client, user_id)
...
class TestPostUser(TestUserBase):
    ...
    def test_creates_new_user(self, _):
        user_data = get_sample_new_user_data()
        ...
        expected_data = get_sample_existing_user_data(id="stub_id")
...
class TestGetUser(TestUserBase):
    ...
    def _add_sample_user(self, **overrides):
        user_data = get_sample_new_user_data(**overrides)
    ...
    def test_gets_user(self):
        ...
        self.assertEqual(get_sample_existing_user_data(id=user_id), json.loads(response.data))

    def test_gets_all_users(self):
        user1 = get_sample_new_user_data()
        user2 = get_sample_new_user_data(username="tu2",
        ...
        expected_user1 = get_sample_existing_user_data(**user1)
        expected_user2 = get_sample_existing_user_data(**user2)
